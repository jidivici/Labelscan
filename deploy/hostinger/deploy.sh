...
reset_demo_data() {
  ...
  # LabelScan mount; do not remove any Compose, proxy, database, or TLS volume.
  printf '==> Removing existing raw LabelScan objects\n'
  docker compose -f "$COMPOSE_FILE" run --rm --no-deps \
    --entrypoint /bin/sh api -c 'find /app/data/raw -depth -mindepth 1 -delete'

  # PostgreSQL does not permit DROP DATABASE while application connections are
  # open.  Run each statement separately so DROP/CREATE are never wrapped in a
  # transaction, then let the normal migration and idempotent demo seed rebuild
  # the complete schema and known sample data.
...
rollback_image() {
  ...
  if [[ "$reset_demo" -eq 1 ]]; then
    # A demo reset replaces both PostgreSQL and raw objects.  Restoring only the
    # prior image would combine an old binary with a new schema/data set, so the
    # VPS snapshot taken before the reset is the coherent recovery point.
    printf '==> Reset deployment failed; prior image was not restored because the data reset is not image-rollback-safe\n' >&2
    exit "$exit_code"
  fi
